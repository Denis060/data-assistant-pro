...
def detect_outliers_modified_zscore(data, threshold=3.5):
    ...
    modified_z_scores = 0.6745 * (data - median_val) / mad if mad != 0 else np.zeros_like(data)
...
def show_outlier_analysis(df, col, detection_method, **kwargs):
    ...
    if outlier_info['total_outliers'] > 0:
        ...
        outlier_details = pd.DataFrame({
            'Index': outlier_info['indices'],
            'Value': outlier_info['values']
        })
        
        # Add method-specific details
        if detection_method == 'Z-Score' and 'z_scores' in outlier_info:
            outlier_details['Z-Score'] = outlier_info['z_scores']
        elif detection_method == 'Modified Z-Score' and 'modified_z_scores' in outlier_info:
            ...
            for val in outlier_info['values']:
                if val < outlier_info['lower_bound']:
                    distances.append(f"Below by {outlier_info['lower_bound'] - val:.2f}")
                else:
                    distances.append(f"Above by {val - outlier_info['upper_bound']:.2f}")
            outlier_details['Distance from Bound'] = distances
        
        # Show outlier table (limit to first 20 for display)
        if len(outlier_details) > 20:
            st.write(f"Showing first 20 outliers (out of {len(outlier_details)} total):")
    ...
    fig = make_subplots(
        rows=2, cols=2,
        subplot_titles=('Distribution Histogram', 'Box Plot with Outliers', 
                       'Data Points (Outliers Highlighted)', 'Statistical Summary'),
    ...
    # Histogram
    fig.add_trace(
        go.Histogram(x=data, name="Data Distribution", nbinsx=30, 
    ...
    fig.add_trace(
        go.Scatter(x=normal_data.index, y=normal_data, 
    ...
    if len(outlier_data) > 0:
        fig.add_trace(
            go.Scatter(x=outlier_data.index, y=outlier_data,
    ...
    if detection_method == 'IQR':
        ...
        summary_data = [
            ...
            ['Upper Limit', f"{outlier_info['mean'] + outlier_info['threshold'] * outlier_info['std']:.2f}"],
            ['Lower Limit', f"{outlier_info['mean'] - outlier_info['threshold'] * outlier_info['std']:.2f}"]
        ]
...
def handle_outliers(df: pd.DataFrame, method: str, detection_method: str, columns: list, **kwargs) -> pd.DataFrame:
    ...
    try:
        ...
        for col in columns:
            ...
            if outlier_info['total_outliers'] == 0:
                st.success(f"✅ No outliers detected in '{col}' using {detection_method} method")
                continue
            ...
            if method == 'Remove':
                ...
                if detection_method == 'IQR':
                    ...
                    df_cleaned.loc[outlier_mask & (df_cleaned[col] < lower_bound), col] = lower_bound
                    df_cleaned.loc[outlier_mask & (df_cleaned[col] > upper_bound), col] = upper_bound
                    ...
                    df_cleaned.loc[outlier_mask & (df_cleaned[col] < lower_percentile), col] = lower_percentile
                    df_cleaned.loc[outlier_mask & (df_cleaned[col] > upper_percentile), col] = upper_percentile
                ...
                mean_val = df[col][~outlier_mask].mean()  # Mean of non-outliers
                ...
                median_val = df[col][~outlier_mask].median()  # Median of non-outliers
            ...
            with col1:
                ...
                st.write(f"• Range: [{initial_stats['min']:.2f}, {initial_stats['max']:.2f}]")
            ...
            with col2:
                ...
                st.write(f"• Range: [{final_stats['min']:.2f}, {final_stats['max']:.2f}]")
            
            # Show changes
            st.write("**Changes:**")
            mean_change = ((final_stats['mean'] - initial_stats['mean']) / initial_stats['mean']) * 100
            std_change = ((final_stats['std'] - initial_stats['std']) / initial_stats['std']) * 100
            st.write(f"• Mean change: {mean_change:+.1f}%")
            st.write(f"• Std Dev change: {std_change:+.1f}%")
            if method == 'Remove':
                st.write(f"• Rows removed: {initial_stats['count'] - final_stats['count']:,}")
